...
$conversions['text'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $string = '%' . str_replace('%', '', $string) . '%';
        $results = array_merge($results, TABLETEMPLATE::_search(['name'], [$string], 'CREATEREGIONCLASS'));
    }
};
$conversions['all'] = function($string) {
    global $results,$arguments;
    if (count($results) > 0) return;
    $results = TABLETEMPLATE::_all('CREATEREGIONCLASS');
};
$conversions['starts_with'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $arguments['SEARCHDESCRIPTION'] .= ' Start With \''.$string.'\'';
        $string = str_replace('%', '', $string) . '%';
        $results = array_merge($results, TABLETEMPLATE::_search(array('name'), array($string), 'CREATEREGIONCLASS'));
    }
};
...
$conversions['region'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $arguments['SEARCHDESCRIPTION'] .= ' Regional Dex: \''.$region->Name().'\'';
        $string = $region->Id();
        $results = TABLETEMPLATE::_get('CREATEREGIONCLASS', ['appearance_region'], [$string], 'ORDER BY number LIMIT '.$_GET['o'].',30');
    }
};
$conversions['user'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $arguments['SEARCHDESCRIPTION'] .= ' Created By \''.$string.'\'';
        $string = $user->Id();
        $results = TABLETEMPLATE::_search(array('uid'), array($string), 'CREATEREGIONCLASS');
    }
};
function print_result($objects,$twigfile) {
    ...
    foreach ($objects as $i) {
        if (!empty($i->Id())) {
            $data = $i->data;
            $data['description'] = substr($data['description'],0,60).'...';
...
if (isset($_GET['by']) && strlen(str_replace('%', '', $_GET['by'])) >= 2) {
    ...
    $arguments['RESULTS'] = print_result($results,"html/_templates/Search/CREATEREGIONCLASS.twig");
