...
$conversions['text'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $string = '%' . str_replace('%', '', $string) . '%';
        $results = array_merge($results, TABLETEMPLATE::_search(['name'], [$string], 'CREATEABILITYCLASS'));
        $results = array_merge($results, TABLETEMPLATE::_search(['description'], [$string], 'CREATEABILITYCLASS'));
    }
};
$conversions['all'] = function($string) {
    global $results,$arguments;
    if (count($results) > 0) return;
    $results = TABLETEMPLATE::_all('CREATEABILITYCLASS');
};
$conversions['starts_with'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $arguments['SEARCHDESCRIPTION'] .= ' Start With \''.$string.'\'';
        $string = str_replace('%', '', $string) . '%';
        $results = array_merge($results, TABLETEMPLATE::_search(array('name'), array($string), 'CREATEABILITYCLASS'));
    }
};
...
$conversions['region'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $arguments['SEARCHDESCRIPTION'] .= ' Regional Dex: \''.$region->Name().'\'';
        $string = $region->Id();
        $results = TABLETEMPLATE::_get('CREATEABILITYCLASS', ['appearance_region'], [$string], 'ORDER BY number LIMIT '.$_GET['o'].',30');
    }
};
$conversions['user'] = function($string) {
    ...
    if (count($results) > 0) {
        ...
        $arguments['SEARCHDESCRIPTION'] .= ' Created By \''.$string.'\'';
        $string = $user->Id();
        $results = TABLETEMPLATE::_search(array('uid'), array($string), 'CREATEABILITYCLASS');
    }
};
...
if (isset($_GET['by']) && strlen(str_replace('%', '', $_GET['by'])) >= 2) {
    ...
    $arguments['RESULTS'] = print_result($results,"html/_templates/Search/CREATEABILITYCLASS.twig");
